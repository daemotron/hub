...
The Stormrose Hub's code has to be compatible with Python 3.6 and newer versions. There is no need for backwards
compatibility to Python's 3.4, 3.3, 3.2, 3.1, 2.x or earlier branches. In consequence, all functions, classes,
...
alias name should be avoided::
   ...
   # not ok
   from hub.something import SomeClass as OtherClass

   # ok
   from hub.something import SomeClass
